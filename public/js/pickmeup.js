(function(root, factory) {
    ...
    function fill(target) {
        ...
        for (var i = 0; i < options.calendars; i++) {
            ...
            dom_query(instance, '.pmu-month').dataset.month=current_date.getMonth()+1;
            dom_query(instance, '.pmu-month').dataset.year=current_date.getFullYear();
    ...
    pickmeup_init.defaults = {
        ...
        instance_template: function(options) {
            ...
            return '<div class="pmu-instance">' +
                ...
                '<div class="2222 pmu-month pmu-button"></div>' +
                '<div class="pmu-one pmu_button">Сегодня</div>' +
                '<div class="pmu-two pmu_button">7 дней</div>' +
                '<div class="pmu-three pmu_button">30 дней</div>' +
...
(function(root, factory) {
    ...
    function dom_on(target, element, event, callback) {
        if (event.indexOf(' ') !== -1) {
            ...
            target.__pickmeop.events.push([element, event, callback]);
            element.addEventListener(event, callback);
        }
    }
    ...
    function dom_off(target, element, event, callback) {
        ...
        if (event && event.indexOf(' ') !== -1) {
            ...
            events = target.__pickmeop.events;
            events_number = events.length;
            for (i = 0; i < events_number; ++i) {
                if (
                    (element && element !== events[i][0]) ||
    ...
    function dom_dispatch_event(element, event, detail) {
        ...
        e.initEvent('pickmeop-' + event, false, true);
        return element.dispatchEvent(e);
    }
     ...
     */
    function fill(target) {
        var root_element = target.__pickmeop.element,
            options = target.__pickmeop.options,
            current_cal = Math.floor(options.calendars / 2),
            actual_date = options.date,
            current_date = options.current,
            min_date = options.min ? new Date(options.min) : null,
        ...
        for (var i = 0; i < options.calendars; i++) {
            ...
            (function() {
                ...
                for (j = 0; j < 12; ++j) {
                    ...
                    year_element = document.createElement('div');
                    year_element.textContent = year;
                    year_element.__pickmeop_year = year;
                    if (
                        (options.min && year < min_year) ||
                        (options.max && year > max_year)
                    ) {
            ...
            (function() {
                ...
                for (month = 0; month < 12; ++month) {
                    month_element = document.createElement('div');
                    month_element.textContent = options.locales[options.locale].monthsShort[month];
                    month_element.__pickmeop_month = month;
                    month_element.__pickmeop_year = current_year;
                    if (
                        (
                            options.min &&
                            (
            ...
            (function() {
                ...
                for (day = 0; day < 42; ++day) {
                    day_element = document.createElement('div');
                    day_element.textContent = local_date.getDate();
                    day_element.__pickmeop_day = local_date.getDate();
                    day_element.__pickmeop_month = local_date.getMonth();
                    day_element.__pickmeop_year = local_date.getFullYear();
                    if (current_month !== local_date.getMonth()) {
                        dom_add_class(day_element, 'pmu-not-in-month');
                    }
                    if (local_date.getDay() === 0) {
     ...
     */
    function update_date(target, new_date) {
        var options = target.__pickmeop.options,
            i;
        reset_time(new_date);
        (function() {
            var new_value;
    ...
    function click(target, event) {
        ...
        if (dom_matches(element.parentElement, 'nav')) {
            ...
            if (dom_has_class(root, 'pmu-view-years')) {
                options.current.setFullYear(element.__pickmeop_year);
                if (options.select_month) {
                    dom_remove_class(root, 'pmu-view-years');
                    dom_add_class(root, 'pmu-view-months');
                } else if (options.select_day) {
                ...
                options.current.setMonth(element.__pickmeop_month);
                options.current.setFullYear(element.__pickmeop_year);
                if (options.select_day) {
                    dom_remove_class(root, 'pmu-view-months');
                    dom_add_class(root, 'pmu-view-days');
                ...
                var new_date = new Date(options.current);
                new_date.setYear(element.__pickmeop_year);
                new_date.setMonth(element.__pickmeop_month);
                new_date.setDate(element.__pickmeop_day);
                options.bound.update_date(new_date);
            }
        }
        options.bound.fill();
     ...
     * @return {(Object|null)} Object with useful methods on success, `null` otherwise
     */
    function pickmeop_init(target, initial_options) {
        if (typeof target == 'string') {
            target = document.querySelector(target);
        }
        if (!target) {
        ...
        if (!target.__pickmeop) {
            var i,
                option,
                options = {};
            initial_options = initial_options || {};
            for (i in pickmeop_init.defaults) {
                options[i] = i in initial_options ? initial_options[i] : pickmeop_init.defaults[i];
            }
            for (i in options) {
                option = target.getAttribute('data-pmu-' + i);
                if (option !== null) {
            ...
            element.__pickmeop_target = target;
            dom_add_class(element, 'pickmeop');
            if (options.class_name) {
                dom_add_class(element, options.class_name);
            }
            options.bound = {
    ...
    pickmeop_init.defaults = {
        ...
        instance_template: function(options) {
            ...
            return '<div class="pmu-instance">' +
                '<nav>' +
                '<div class="pmu-prev pmu-button">' + options.prev + '</div>' +
                '<div class="1111 pmu-month pmu-button"></div>' +
                '<div class="pmu-next pmu-button">' + options.next + '</div>' +
                '</nav>' +
                '<nav class="pmu-day-of-week"><div>' + days_of_week.join('</div><div>') + '</div></nav>' +
                '</div>';
