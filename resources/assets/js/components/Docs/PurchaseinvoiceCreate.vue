  ...
  <div>
    <div class="row" v-for="(item, ind) in items" :key="ind + 'stock'">
      ...
      <div class="col-xs-6 col-sm-1 col-xs-xs-xs-12 sto">
        <button @click.prevent="remove('stock', ind)" class="delite">
          <img src="/img/delite.png" alt="delite" />
        </button>
      </div>
    </div>
    <div class="row" v-for="(item, index2) in item_ings" :key="index2 + 'ing'">
      ...
      <div class="col-xs-6 col-sm-1 col-xs-xs-xs-12 sto">
        <button @click.prevent="remove('ing', index2)" class="delite">
          <img src="/img/delite.png" alt="delite" />
        </button>
      </div>
    </div>
    <div class="row">
      <div class="col-xs-12 reason-btn">
        <input
          class="add_component"
          type="submit"
          @click.prevent="add('stock')"
          :value="$t('AddProduct')"
        />
        <input
          class="add_component"
          type="submit"
          @click.prevent="add('ing')"
          :value="$t('AddIng')"
        />
        <input
          class="create_invoice"
          type="submit"
          :value="$t('CreateInvoice')"
        />
      </div>
    </div>
  </div>
...
export default {
  ...
  created() {
    axios
      .get("/doc/getProducts", { params: { resolve: false } })
      .then((response) => {
