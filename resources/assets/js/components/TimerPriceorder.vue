...
export default {
  ...
  methods: {
    getPrice() {
      axios
        .get("/ajax/priceorder" + "?order_id=" + this.order_id)
        .then((response) => {
          this.priceOrder = response.data.results.priceOrder;
          this.priceOrderTotal =
            Math.ceil(response.data.results.priceOrderTotal * 100) / 100;
          this.priceOrderDiscount =
            Math.ceil(response.data.results.priceOrderDiscount * 100) / 100;
          if (!this.first) {
            this.first = true;
            setInterval(() => {
              this.getPrice();
